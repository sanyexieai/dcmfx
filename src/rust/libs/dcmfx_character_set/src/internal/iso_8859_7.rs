...
const LOOKUP_TABLE: [u16; 256] = [
  ...
  0x00AB, 0x00AC, 0x00AD, 0xFFFD, 0x2015, 0x00B0, 0x00B1, 0x00B2, 0x00B3,
  ...
  0x039F, 0x03A0, 0x03A1, 0xFFFD, 0x03A3, 0x03A4, 0x03A5, 0x03A6, 0x03A7,
...
mod tests {
  ...
  fn decode_next_codepoint_test() {
    let result = decode_next_codepoint(&[0xD9]).unwrap();

    assert_eq!(result.0, '\u{03A9}');
    assert_eq!(result.1, []);
  }
}
